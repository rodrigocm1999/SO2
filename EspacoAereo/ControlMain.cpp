...
bool ControlMain::add_airport(const TCHAR* name, int x, int y)
{
	...
	pair<AIRPORT_ID, Airport*> pair(airport->id, airport);
...
void ControlMain::board_people(PLANE_ID plane_offset, AIRPORT_ID origin_airport_id, AIRPORT_ID destiny_airport_id) {
	...
	Airport* destiny = get_airport(origin_airport_id);

	std::vector<Passenger*>* list = origin->remove_passengers(destiny->id);
	flying_passengers_map.insert(pair<PLANE_ID, vector<Passenger*>*>(plane_offset, list));
}
